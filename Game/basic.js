...
//     ctx.drawImage(L,canvas.width*0.9,450c50,50);
// },1);
}
...
window.addEventListener('keydown', function (event) {
    ...
    if (event.keyCode == 32) { pauseGame('PAUSED'); }
...
if(object[i].score==1)
{
    result++;
                ...
                object.push(new Circle(object[i].x, object[i].y, object[i].dx,object[i].dy,object[i].radius*0.5 +1,Math.round(object[i].initial*0.5)));
                object.push(new Circle(object[i].x, object[i].y, -object[i].dx,-object[i].dy, object[i].radius*0.5+1,Math.round(object[i].initial*0.5)));
                            }
                            else{
...
explode.play();
                                object.push(new Circle(object[i].x, object[i].y, object[i].dx,object[i].dy,2,Math.round(object[i].initial*0.5),true));
                                object.push(new Circle(object[i].x, object[i].y, -object[i].dx,-object[i].dy,2,Math.round(object[i].initial*0.5),true));}
               object[i].src = '';
               object.splice(i, 1);
                        }
        ...
        if (!X.length) {
            for (var i = 0; i < object.length; i++) { object[i].on = false; }
        }
        this.draw();
    }
...
this.x = x;
   ...
   if(!score)
    this.score = Math.round(Math.random() * 5) + 1;
   else this.score=score;

   this.initial=this.score;
if(!over)
    this.radius =Math.min(Math.max( 10,r + this.score),45) ;
else 
this.radius =5 ;
...
if(this.x<canvas.width-50&&this.dx<0)
this.start=false;
...
messenger();
    ...
    pauseGame('GameOver:~(',"Wanna Try Again...");
    ...
    document.getElementById('play').style.visibility='visible';
        ...
        if (this.y - this.radius + this.dy <= 0)
            this.dy = -1 * this.dy;
        if (this.y + this.radius + this.dy >= 490)
            this.dy = -1 * this.dy;}
...
BACKGROUNDS.src='backgroundD.mp3';
...
var LEVEL=[
    // object.push(new Circle(-1100, 100, 1, 3, 30));
    // object.push(new Circle(-800, 100, 3, 1, 30));
    // object.push(new Circle(-500, 100, 5, 3, 30));
 [
...
bg = new IMAGE('background.jpg', 0, 0, window.innerWidth, 490);
...
// object.push(new Circle(-800, 100, 3, 1, 30));
// object.push(new Circle(-500, 100, 5, 3, 30));
// object.push(new Circle(100, 300, 3, 2, 30));
// object.push(new Circle(300, 200, 4, 5, 30));
// object.push(new Circle(400, 400, 2, 4, 30));
// object.push(new Circle(100, 300, 3, 5, 30));
...
if(Leveln==3){
    if(result==+sessionStorage.getItem('HSCORE'))
    pauseGame('Congratualation For Completing The Game:~)',"Congo You've Made A New Score");
    else
    pauseGame('Congratualation For Completing The Game:~)','Wanna Play Again');
